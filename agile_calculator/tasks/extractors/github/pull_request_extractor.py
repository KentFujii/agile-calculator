...
class PullRequestExtractor(GitHubExtractor):
    def __init__(self, repo_name: str, users: tuple, since_days: int, base_branch: str = "main"):
        super().__init__()
    ...
    def run(self) -> Iterator[PullRequestRecord]:
        repo = self.client.get_repo(self.repo_name)
        pull_requests = repo.get_pulls(
            state="close", sort="created", direction="desc", base=self.base_branch
        )
        return self._extract_request_records(pull_requests)
