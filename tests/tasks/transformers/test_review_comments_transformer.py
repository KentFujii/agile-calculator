...
class TestReviewCommentsTransformer:
    @pytest.fixture
    def transformer(self):
        return ReviewCommentsTransformer()
    ...
    def test_map_records(self, transformer):
        ...
        transforms PullRequestRecord to ReviewCommentsRecord and filters out unmerged PRs.
        """
        today = date.today()
        records = [
            self.create_pr_record(datetime.combine(today, datetime.min.time()), 10),
    ...
    def test_reduce_records(self, transformer):
        ...
        yesterday = today - timedelta(days=1)
        records = [
            ReviewCommentsRecord(merged_date=today, review_comments=10),
            ReviewCommentsRecord(merged_date=yesterday, review_comments=5),
